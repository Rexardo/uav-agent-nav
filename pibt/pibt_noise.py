...
# ==========================================
# PIBT planner with safe distance penalty 
# and dynamic deadlock solver
# ==========================================
class SafePIBTStepPlanner:
    ...
    def _assign(self, index: int) -> bool:
        ...
        # Dynamic collision penalty and noise used to break symmetry
        def candidate_score(c):
            ...
            # Dynamic noise. 0.05 for common time, 1.5 when encounter corridor
            max_noise = 1.5 if is_crowded else 0.05
            noise = self.rng.uniform(0.0, max_noise)
            
            return base_dist + penalty + noise
...
def run_simulation(
    ...
    for t in range(max_logical_steps):
        ...
        for uav in uavs:
            ...
            if uav.is_reached:
                ...
                base_priority = float(abs(uav.pos[0] - target[0]) + abs(uav.pos[1] - target[1]))
    ...
    ax_final.set_title("Final Trajectories of All UAVs (Safe PIBT Variant)")
