...
class UAV:
    ...
    # ==========================================
    # Reference path generation (Dynamic APF)
    # ==========================================
    def plan_reference_path(self, static_map, neighbors_info=None):
        """
        Generate the future reference path using APF.

        Different from the previous APF version, neighbors_info is now passed into
        the planner. Therefore APF considers:
        1) goal attraction;
        2) static-obstacle repulsion;
        3) dynamic UAV repulsion from neighboring UAV positions / predicted paths.
        """
        if self.is_reached:
            return
        ...
        goal_grid = self._clip_to_grid(self.goal, static_map)
        ...
        full_path = apf(
            ...
            external_avoid_penalty=100.0,
            # Receding-horizon simulation: only the next H steps are executed,
            # so a moderate APF iteration budget is enough and faster.
            max_iterations=max(static_map.width + static_map.height, self.horizon * 30),
            return_partial=True,
            escape_radius=12,
        )
    ...
    def _fill_current_path_from_grid_path(self, full_path, start_grid=None):
        """
        Convert a grid path returned by APF into the UAV's rolling horizon path.

        Important change: if APF returns only a partial path, the UAV holds the last
        APF point instead of jumping directly to the final goal.
        """
        self.current_path = []
    ...
    def _plan_to_target_with_dynamic_apf(self, target, static_map, neighbors_info):
        ...
        This replaces the old straight-line movement toward the parking spot, so APF
        also participates during conflict resolution and yielding.
        """
        start_grid = self._clip_to_grid(self.pos, static_map)
            ...
            if self.wait_steps > 3:
                ...
                if self.id > conflict_neighbor['id']:
                    ...
                    if parking_spot is not None:
                        ...
                        # Use dynamic APF instead of a straight line to move toward the parking place.
                        # This makes APF participate in both normal planning and yielding.
                        ...
                        if planned:
                            print(f"[Dynamic APF Yielding] UAV {self.id} planned an APF parking path, yielding to UAV {conflict_neighbor['id']}!")
                        else:
                            self.current_path = [self.pos.copy() for _ in range(self.horizon)]
...
def run_simulation(
    ...
    for t in range(max_logical_steps):
        # 1) Communicate first, then plan. This is the key change for dynamic APF:
        # each UAV passes neighbor positions / predicted paths into the APF planner.
        neighbor_infos = {uav.id: uav.communicate(uavs) for uav in uavs}

        for uav in uavs:
            uav.plan_reference_path(env_map, neighbor_infos[uav.id])

        # 2) Keep the original conflict-resolution layer as a safety backup.
        # APF now handles dynamic repulsion before this stage, so this block should
        # be triggered less often than in the static-only APF version.
        for uav in uavs:
        ...
        for f in range(render_frames_per_step):
            ...
            ax.set_aspect('equal')
            ax.set_title(f"Dynamic-Repulsion APF - Logic Step {t} | Render Frame {f + 1}/{render_frames_per_step}")
            ax.grid(True)
...
if __name__ == "__main__":
    run_simulation(
        ...
        height=50,
        num_obstacles=50,
        map_seed=None,        # 改成 None 就是每次随机生成不同地图；固定数字则可复现实验
        inflation_radius=0.8,
        max_logical_steps=500,
        render_frames_per_step=5,
    )
