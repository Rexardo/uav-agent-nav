...
bool ViewNode::searchPath(
    const Vector3d& p1, const Vector3d& p2, vector<Vector3d>& path, double& path_length) {
...
double ViewNode::computeCost(const Vector3d& p1, const Vector3d& p2, const double& y1,
  ...
  // Cost matrices used by the external TSP solver must remain finite. Keep an unreachable edge
  // prohibitively expensive here; the selected next viewpoint is verified again before execution.
  double pos_cost = 1e5;
  if (ViewNode::searchPath(p1, p2, path, path_length)) pos_cost = path_length / vm_;
