...
void MapROS::init() {
  ...
  node_.param("map_ros/known_grid_height", known_grid_height_, 1.0);
  ...
  known_grid_pub_ = node_.advertise<nav_msgs::OccupancyGrid>("/racer/known_grid", 1, true);
...
void MapROS::publishKnownGrid() {
  // Export RACER's online occupancy buffer, never the simulator's complete PCD.
  ...
  msg.info.width =
      std::max(1, static_cast<int>(std::ceil((box_max.x() - box_min.x()) / resolution)));
  msg.info.height =
      std::max(1, static_cast<int>(std::ceil((box_max.y() - box_min.y()) / resolution)));
  msg.info.origin.position.x = box_min.x();
  msg.info.origin.position.y = box_min.y();
  msg.info.origin.position.z = known_grid_height_;
  ...
  for (unsigned int y = 0; y < msg.info.height; ++y) {
    for (unsigned int x = 0; x < msg.info.width; ++x) {
      const Eigen::Vector3d pos(box_min.x() + (x + 0.5) * resolution,
          box_min.y() + (y + 0.5) * resolution, known_grid_height_);
      const int occupancy = map_->getOccupancy(pos);
      int8_t value = -1;
      if (occupancy == SDFMap::OCCUPIED || map_->getInflateOccupancy(pos) == 1)
        value = 100;
