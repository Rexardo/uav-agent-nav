...
void FastExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  nh.param("exploration/astar_resolution", ep_->astar_resolution_, 0.3);
  nh.param("exploration/astar_search_time", ep_->astar_search_time_, 1.0);
  nh.param("exploration/astar_fallback_resolution", ep_->astar_fallback_resolution_, 0.15);
  nh.param(
      "exploration/astar_fallback_search_time", ep_->astar_fallback_search_time_, 1.0);
  nh.param("exploration/max_viewpoint_trials", ep_->max_viewpoint_trials_, 10);
  ...
  planner_manager_->path_finder_->setResolution(ep_->astar_resolution_);
  planner_manager_->path_finder_->max_search_time_ = ep_->astar_search_time_;
...
int FastExplorationManager::planExploreMotion(
  ...
  Vector3d next_pos;
  double next_yaw;
  vector<Vector3d> fallback_points;
  vector<double> fallback_yaws;
  auto appendFallbackViews = [&](const vector<vector<Vector3d>>& points,
                                 const vector<vector<double>>& yaws) {
    size_t max_view_num = 0;
    for (size_t i = 0; i < points.size() && i < yaws.size(); ++i)
      max_view_num = std::max(max_view_num, std::min(points[i].size(), yaws[i].size()));

    // Round-robin by frontier, so a difficult first frontier cannot consume every trial.
    for (size_t j = 0; j < max_view_num; ++j) {
      for (size_t i = 0; i < points.size() && i < yaws.size(); ++i) {
        if (j >= points[i].size() || j >= yaws[i].size()) continue;
        fallback_points.push_back(points[i][j]);
        fallback_yaws.push_back(yaws[i][j]);
      }
    }
  };
  ...
  if (grid_ids.empty()) {
    ...
    if (min_cost_id < 0) {
      ROS_WARN("Assigned grids contain no reachable frontier candidate");
      return NO_REACHABLE_VIEW;
    }
    next_pos = ed_->points_[min_cost_id];
    next_yaw = ed_->yaws_[min_cost_id];
    ...
    if (ep_->refine_local_) {
      ...
      appendFallbackViews(ed_->n_points_, n_yaws);

      if (ed_->n_points_.empty() || ed_->n_points_[0].empty()) {
        ROS_WARN("No viewpoint candidate for assigned frontier %d", frontier_ids[0]);
        return NO_REACHABLE_VIEW;
      }
      ...
      if (grid_ids.size() <= 1) {
        ...
        if (!ed_->refined_points_.empty()) {
          ...
          next_pos = fallback_points.front();
          next_yaw = fallback_yaws.front();
        }
      }
    ...
    if (fallback_points.empty()) {
      ROS_WARN("No viewpoint candidate in this drone's assigned frontiers");
      return NO_REACHABLE_VIEW;
    }
  ...
  // Try the locally optimized viewpoint first, then the remaining viewpoints from the locally
  // refined frontiers assigned to this drone. This preserves the swarm allocation while avoiding
  // an infinite retry on one unreachable sample.
  vector<Vector3d> candidate_points = { next_pos };
  vector<double> candidate_yaws = { next_yaw };
  for (int i = 0; i < fallback_points.size(); ++i) {
    bool duplicate = false;
    for (const auto& candidate : candidate_points) {
      if ((candidate - fallback_points[i]).norm() < 1e-3) {
        duplicate = true;
        break;
      }
    }
    if (!duplicate) {
      candidate_points.push_back(fallback_points[i]);
      candidate_yaws.push_back(fallback_yaws[i]);
    }
  }

  const int trial_num = std::min(int(candidate_points.size()), ep_->max_viewpoint_trials_);
  for (int i = 0; i < trial_num; ++i) {
    std::cout << "Try view " << i + 1 << "/" << trial_num << ": "
              << candidate_points[i].transpose() << ", " << candidate_yaws[i] << std::endl;
    ed_->next_pos_ = candidate_points[i];
    ed_->next_yaw_ = candidate_yaws[i];
    if (planTrajToView(pos, vel, acc, yaw, candidate_points[i], candidate_yaws[i]) == SUCCEED) {
      double total = (ros::Time::now() - t2).toSec();
      ROS_INFO("Total time: %lf", total);
      ROS_ERROR_COND(total > 0.1, "Total time too long!!!");
      return SUCCEED;
    }
    ROS_WARN("Viewpoint %d/%d is unreachable, trying another assigned candidate", i + 1,
        trial_num);
  }
  ROS_WARN("No reachable viewpoint remains in this drone's locally assigned frontiers");
  return NO_REACHABLE_VIEW;
}
...
int FastExplorationManager::planTrajToView(const Vector3d& pos, const Vector3d& vel,
  ...
  auto* path_finder = planner_manager_->path_finder_.get();
  path_finder->setResolution(ep_->astar_resolution_);
  path_finder->max_search_time_ = ep_->astar_search_time_;
  path_finder->reset();
  int search_result = path_finder->search(pos, next_pos, optimistic);

  if (search_result != Astar::REACH_END) {
    ROS_WARN("Primary A* failed (%s), retry at %.2f m resolution",
        search_result == Astar::SEARCH_TIMEOUT ? "timeout" : "no path",
        ep_->astar_fallback_resolution_);
    path_finder->setResolution(ep_->astar_fallback_resolution_);
    path_finder->max_search_time_ = ep_->astar_fallback_search_time_;
    path_finder->reset();
    search_result = path_finder->search(pos, next_pos, optimistic);
  }

  // Always restore the primary search configuration before returning to other planner code.
  path_finder->setResolution(ep_->astar_resolution_);
  path_finder->max_search_time_ = ep_->astar_search_time_;
  if (search_result != Astar::REACH_END) {
    ROS_ERROR("No path to next viewpoint after refined A* (%s)",
        search_result == Astar::SEARCH_TIMEOUT ? "timeout" : "no path");
    return FAIL;
  }
  ed_->path_next_goal_ = path_finder->getPath();
...
void FastExplorationManager::refineLocalTour(const Vector3d& cur_pos, const Vector3d& cur_vel,
  ...
  for (auto pt : refined_pts) {
    ...
    if (ViewNode::searchPath(ed_->refined_tour_.back(), pt, path, path_length))
