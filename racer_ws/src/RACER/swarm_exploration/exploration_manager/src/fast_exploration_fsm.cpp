...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  nh.param("fsm/unreachable_release_count", fp_->unreachable_release_count_, 3);
  ...
  fd_->unreachable_view_count_ = 0;
  fd_->next_plan_attempt_time_ = ros::Time(0);
  ...
  // Out-of-band experiment state; it is not consumed by RACER itself.
  fsm_state_pub_ = nh.advertise<std_msgs::UInt8>("/experiment/fsm_state", 1, true);
  planning_time_pub_ = nh.advertise<std_msgs::Float64>("/experiment/planning_time", 100);
  std_msgs::UInt8 initial_state;
  initial_state.data = static_cast<uint8_t>(state_);
  fsm_state_pub_.publish(initial_state);
...
void FastExplorationFSM::FSMCallback(const ros::TimerEvent& e) {
  ...
  switch (state_) {
    ...
    case PLAN_TRAJ: {
      if (ros::Time::now() < fd_->next_plan_attempt_time_) break;
      ...
      if (res == SUCCEED) {
        ...
        const double retry_delay = std::min(0.2 * fd_->plan_fail_count_, 2.0);
        fd_->next_plan_attempt_time_ = ros::Time::now() + ros::Duration(retry_delay);
        ROS_WARN("Plan fail, retry in %.1f s", retry_delay);
      } else if (res == NO_REACHABLE_VIEW) {
        fd_->static_state_ = true;
        ++fd_->plan_fail_count_;
        ++fd_->unreachable_view_count_;
        const double retry_delay = std::min(0.5 * fd_->unreachable_view_count_, 3.0);
        fd_->next_plan_attempt_time_ = ros::Time::now() + ros::Duration(retry_delay);
        ROS_WARN("No reachable viewpoint in assigned frontiers, refresh map and retry in %.1f s",
            retry_delay);

        // Refresh frontiers before the next attempt. The assignment itself is preserved; pairwise
        // allocation can still update it through the existing swarm callbacks.
        if (expl_manager_->updateFrontierStruct(fd_->odom_pos_) == 0) {
          fd_->last_check_frontier_time_ = ros::Time::now();
          transitState(IDLE, "FSM");
          visualize(1);
        } else if (expl_manager_->ep_->drone_num_ > 1 &&
                   fd_->unreachable_view_count_ >= fp_->unreachable_release_count_) {
          // The first grid is the one whose local frontier candidates were just exhausted. Release
          // only this grid, not the drone's whole assignment. It becomes a missed grid and is
          // picked up by the existing pairwise ACVRP allocation.
          auto& own_state = expl_manager_->ed_->swarm_state_[getId() - 1];
          if (!own_state.grid_ids_.empty()) {
            const int released_grid = own_state.grid_ids_.front();
            own_state.grid_ids_.erase(own_state.grid_ids_.begin());
            own_state.recent_attempt_time_ = 0.0;
            expl_manager_->ed_->reallocated_ = true;
            fd_->plan_fail_count_ = 0;
            fd_->unreachable_view_count_ = 0;
            fd_->next_plan_attempt_time_ = ros::Time(0);
            ROS_WARN("Release unreachable grid %d for pairwise reallocation", released_grid);
...
void FastExplorationFSM::droneStateTimerCallback(const ros::TimerEvent& e) {
  ...
  // Heartbeat for the experiment recorder. The per-drone launch remap keeps
  // this measurement channel separate from RACER's swarm communication.
  std_msgs::UInt8 state_msg;
  state_msg.data = static_cast<uint8_t>(state_);
  fsm_state_pub_.publish(state_msg);
}
...
void FastExplorationFSM::optTimerCallback(const ros::TimerEvent& e) {
  ...
  // The new allocation must cover every optimized grid exactly once. This is the safety check;
  // comparing only travel costs is not valid when missed grids or ACVRP capacity constraints are
  // involved.
  std::unordered_set<int> expected_ids(opt_ids.begin(), opt_ids.end());
  std::unordered_set<int> allocated_ids;
  bool duplicate_id = false;
  for (auto id : ego_ids) duplicate_id |= !allocated_ids.insert(id).second;
  for (auto id : other_ids) duplicate_id |= !allocated_ids.insert(id).second;
  if (duplicate_id || allocated_ids != expected_ids ||
      !std::isfinite(cur_app1 + cur_app2)) {
    ROS_ERROR("Reject invalid reallocation: expected %zu grids, received %zu",
        expected_ids.size(), allocated_ids.size());
    return;
  }

  if (cur_app1 + cur_app2 > prev_app1 + prev_app2 + 0.1) {
    // The old assignment does not contain newly found missed grids, and ACVRP also enforces an
    // unknown-cell capacity constraint. A larger pure path cost is therefore not a reason to
    // reject an otherwise complete, feasible allocation.
    ROS_WARN("Accept reallocation with larger path cost: previous %.3f, current %.3f, missed %zu",
        prev_app1 + prev_app2, cur_app1 + cur_app2, missed.size());
  }
